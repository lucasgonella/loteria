...
df = pd.read_excel(file_path, sheet_name='MEGA SENA')

historico = set(
    tuple(sorted(row)) for row in df[['Bola1', 'Bola2', 'Bola3', 'Bola4', 'Bola5', 'Bola6']].values
)

def mostrar_estatisticas():
    """Mostra estatísticas básicas dos números mais e menos sorteados."""
    numeros = df[['Bola1', 'Bola2', 'Bola3', 'Bola4', 'Bola5', 'Bola6']].values.flatten()
...
def gerar_combinacoes(criterio='mais', tamanho=6, quantidade=1):
...
def gerar_jogos_manualmente(numeros_inseridos, tamanho=6, quantidade=1):
...
def menu():
    ...
    while True:
        ...
        if escolha == '1':
            ...
            tamanho = int(input("Quantos números no jogo (6, 7 ou 8)? "))
